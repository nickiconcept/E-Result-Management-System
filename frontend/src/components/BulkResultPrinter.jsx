...
export default function BulkResultPrinter({ classes, sessions, currentTerm, currentSession, settings, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content glass-panel" style={{ maxWidth: '1000px', backgroundColor: '#fff', color: '#000', padding: '24px' }}>
        
        {/* Controls & Action Bar */}
        <div style={{ borderBottom: '2px solid #e2e8f0', paddingBottom: '16px', marginBottom: '20px' }} className="no-print">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            <div>
              <h2 style={{ margin: 0, fontSize: '1.25rem', color: '#1e40af' }}>🖨️ Admin Bulk A4 Result Printer</h2>
              <p style={{ margin: '4px 0 0 0', fontSize: '0.85rem', color: '#64748b' }}>
                Print all student report cards for a class arm in one continuous batch formatted for A4 paper.
              </p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr auto', gap: '15px', alignItems: 'flex-end' }}>
            <div>
            ...
            >
              🖨️ Print All {bulkData.length} Results (A4)
            </button>
          </div>
        </div>
        ...
        {loading && (
          <div style={{ padding: '30px', textAlign: 'center', color: '#1e40af' }} className="no-print">
            <h3>Generating A4 Report Cards for {selectedClassName}...</h3>
          </div>
        )}
        ...
        {!loading && bulkData.length > 0 && (
          <div className="bulk-print-container print-area">
            {bulkData.map((studentReportData, idx) => (
              <div key={idx} className="bulk-single-card-page">
                <ReportCard
